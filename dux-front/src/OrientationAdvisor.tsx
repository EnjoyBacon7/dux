...
export default function OrientationAdvisor() {
  ...
  if (loading) {
    return (
      <>
        <Header />
        <main className={`nb-page ${styles["advisor-container"]}`}>
      ...
      <main className={`nb-page ${styles["advisor-container"]}`}>
        ...
        <div className={styles["advisor-main"]}>
          ...
          <div className={styles["advisor-input-zone"]}>
            {messages.length === 0 && (
              <div className={styles["advisor-intent-overlay"]}>
                ...
                {intent === "cover_letter" && (
                  <div className={styles["advisor-job-picker"]}>
                    <label htmlFor="advisor-job-select">{t("advisor.select_job") || "Select a job for the cover letter"}</label>
                    <select
                    ...
                    >
                      ...
                      {jobIdFromUrl && !(context?.favouriteJobs ?? []).some((j) => j.jobId === jobIdFromUrl) && (
                        <option value={jobIdFromUrl}>{t("advisor.current_job") || "Current job (from link)"}</option>
                      )}
